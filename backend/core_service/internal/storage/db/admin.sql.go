...
WHERE (
    ($1::booking_status IS NULL OR b.status = $1) AND
    ($2::uuid IS NULL OR b.client_id = $2) AND
    ($3::uuid IS NULL OR s.performer_id = $3)
)
`
...
type CountAllBookingsParams struct {
	...
	PerformerID uuid.NullUUID     `json:"performer_id"`
}

func (q *Queries) CountAllBookings(ctx context.Context, arg CountAllBookingsParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, countAllBookings, arg.Status, arg.ClientID, arg.PerformerID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) CountAllReviews(ctx context.Context, arg CountAllReviewsParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, countAllReviews, arg.Column1, arg.Rating)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
FROM users
WHERE (
    ($1::user_role IS NULL OR role = $1) AND
    ($2::verification_status IS NULL OR verification_status = $2) AND
    ($3::text IS NULL OR name ILIKE '%' || $3 || '%' OR email ILIKE '%' || $3 || '%')
)
`
...
func (q *Queries) CountUsersWithFilters(ctx context.Context, arg CountUsersWithFiltersParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, countUsersWithFilters, arg.Role, arg.VerificationStatus, arg.Search)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
JOIN services s ON s.id = b.service_id
WHERE (
    ($3::booking_status IS NULL OR b.status = $3) AND
    ($4::uuid IS NULL OR b.client_id = $4) AND
    ($5::uuid IS NULL OR s.performer_id = $5)
)
...
func (q *Queries) GetAllServices(ctx context.Context, arg GetAllServicesParams) ([]GetAllServicesRow, error) {
	rows, err := q.db.QueryContext(ctx, getAllServices, arg.Column1, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
FROM users
WHERE (
    ($3::user_role IS NULL OR role = $3) AND
    ($4::verification_status IS NULL OR verification_status = $4) AND
    ($5::text IS NULL OR name ILIKE '%' || $5 || '%' OR email ILIKE '%' || $5 || '%')
)
...
func (q *Queries) GetUsersWithFilters(ctx context.Context, arg GetUsersWithFiltersParams) ([]GetUsersWithFiltersRow, error) {
	rows, err := q.db.QueryContext(ctx, getUsersWithFilters,
		arg.Limit,
		arg.Offset,
		arg.Role,
